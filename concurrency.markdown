...
[递归锁](http://en.wikipedia.org/wiki/Reentrant_mutex)
...
spin\_lock\_softirq()(include/linux/spinlock.h)。它禁用当前cpu的软中断，然
后获取spinlock。
...
spin\_lock\_irq()和spin\_unlock\_irq()。
...
多一些，所以如果使用不当的话，反而效果不好。实际操作时rwlock\_t通常没有太大的
用处。
